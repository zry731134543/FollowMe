...
//        Color.rgb(0xe9, 0xe9, 0xe9)
        // 设置画笔相关属性
        mPaint.setAntiAlias(true);
        yPaint.setAntiAlias(true);
        mPaint.setColor(Color.rgb(183, 130, 135));
        yPaint.setColor(Color.rgb(255, 135, 123));
        canvas.drawColor(Color.TRANSPARENT);
        mPaint.setStrokeWidth(mCircleLineStrokeWidth);
        mPaint.setStyle(Paint.Style.STROKE);
        yPaint.setStrokeWidth(mCircleLineStrokeWidth);
        ...
        canvas.drawArc(mRectF, -90, ((float) mProgress / mMaxProgress) * 360, false, yPaint);

        // 绘制进度文案显示
        mPaint.setStrokeWidth(mTxtStrokeWidth);
        String text = mProgress + "%";
        int textHeight = height / 10;
        int mtextHeight = height / 7;
        mPaint.setTextSize(textHeight);
        int textWidth = (int) mPaint.measureText(text, 0, text.length());
        mPaint.setStyle(Paint.Style.FILL);
        mPaint.setColor(Color.rgb(0xf8, 0x60, 0x30));
        canvas.drawText(text, width / 2 - textWidth / 2, height / 2 + textHeight / 2 -40, mPaint);
        canvas.drawText("目标1000", width / 2 - textWidth / 2 - 40, height / 2 + mtextHeight / 2, mPaint);
